...
namespace IntegrationService.Messaging.Http
{
    public class CategoryAccountClient : ICategoryAccountClient
    {
        ...
        {
            var response = await _httpClient.GetFromJsonAsync<List<CategoryNameDTO>>($"http://categoryaccountservice:8080/api/v1.0/categories/{userId}", cancellationToken);
        ...
        {
            var response = await _httpClient.GetFromJsonAsync<List<AccountNameDTO>>($"http://categoryaccountservice:8080/api/v1.0/accounts/{userId}", cancellationToken);
        ...
        public async Task<AccountNameDTO> CreateAccountAsync(CreateAccountRequest createAccountRequest, CancellationToken cancellationToken)
        {
            ...
            var response = await _httpClient.PostAsJsonAsync("http://categoryaccountservice:8080/api/v1.0/accounts"!!!!/*Поменять эндпоинты на правильные для users запросов*/, createAccountRequest, cancellationToken);
            response.EnsureSuccessStatusCode();
